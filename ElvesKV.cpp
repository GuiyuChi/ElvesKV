...
// 生成字符串的长度
static size_t randValueSize(int strSize) {
   return (size_t)rand()%strSize;
}
// 生成随机字符串
static string randString(int strSize) {
  const size_t value_size = randValueSize(strSize);
...
int main() {
  ...
  clock_t t0 = clock();
  levelDbTest(1000000,5);
  cout << "levelDb time elapsed: " << (clock() - t0) * 1.0e-6 << " seconds"
       << endl;
  // wisckeySet(wk,key2,value2);
  clock_t t1 = clock();
  wiscKeyTest(1000000,5);
  cout << "wiscKey time elapsed: " << (clock() - t1) * 1.0e-6 << " seconds"
       << endl;
  return 0;
}
